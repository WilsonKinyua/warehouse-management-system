          ...
          if (mysqli_num_rows($res) > 0) {
              ...
              if ($row['quantity_available']<$quantity) {
                  ...
                  if ($payment_type == "cheque") {
                      $conn->query("INSERT INTO `payments`(`type`,`transaction_type`, `amount`,`cheque_number`, `date`,`doc_number`) VALUES ('cheque','Sale payment','$amount','$c_code','$date', '$doc_number')")or die($conn->error);
                  }elseif ($payment_type == "mpesa") {
                      $conn->query("INSERT INTO `payments`(`type`, `transaction_type`, `amount`, `mpesa_code`,`date`,`doc_number`) VALUES ('mpesa','Sale payment',$amount,'$c_code','$date', '$doc_number')") or die($conn->error);
                  }
  ...
  if(isset($_POST['save_print'])){
    ...
    if ($valid){
        ...
        while($row=$query->fetch_assoc()){
            ...
            $quantity = $_SESSION['sales_cart'][$row['id']]['quantity'];
            //Check if the product exists
            $res = $conn->query("SELECT quantity_avasilable FROM stock WHERE product='$id'") or die($conn->error);
            if (mysqli_num_rows($res) > 0) {
                // Update Stock
                $row = $res->fetch_array();
                if ($row['quantity_available']<$quantity) {
