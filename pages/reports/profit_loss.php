 ...
 <td><div id="load" class="loader"></div></td>
                ...
                <table id="Sales_table" class="table table-bordered table-striped"  style="overflow-x:auto;">
                    ...
                    if ($_SERVER["REQUEST_METHOD"] == "POST") {
                        ...
                        if($_POST['id'] == 'all' and $_POST["store_code"] == "all") {
                            ...
                            $res = $conn->query(
                                ...
                                "SELECT products.name, SUM(new_sale.quantity) AS quantity, FORMAT(SUM(new_sale.quantity * products.price),2) AS Cost_Price ,FORMAT(SUM(new_sale.quantity * new_sale.sale_price),2) AS Selling_Price,
                                FORMAT((SUM(new_sale.quantity * new_sale.sale_price) - SUM(new_sale.quantity * products.price)),2) AS Profit
                                FROM new_sale
                                INNER JOIN products ON new_sale.product=products.id WHERE `salesman` IN(SELECT id FROM users) AND new_sale.date BETWEEN CAST('$from' AS DATE) AND DATE_ADD(CAST('$to' AS DATE), INTERVAL 1 DAY)
                            ...
                            while ($row = $res->fetch_assoc()) {
                                ...
                                $profit_clean = str_replace(",", '', $row['Profit']);
                                $clean_cost_price = str_replace(",", '', $row['Cost_Price']);
                                $clean_selling_price = str_replace(",", '', $row['Selling_Price']);

                                $tot["TOTAL_PROFIT"] += (int)$profit_clean;
                                $tot["TOTAL_COST_PRICE"] += (int)$clean_cost_price;
                                $tot["TOTAL_SELLING_PRICE"] += (int)$clean_selling_price;
                            ...
                            $res = $conn->query(
                                ...
                                WHERE `salesman`='$id' AND new_sale.date BETWEEN CAST('$from' AS DATE) AND DATE_ADD(CAST('$to' AS DATE), INTERVAL 1 DAY)
                                GROUP BY  products.name 
                                ORDER BY products.name"
                            ...
                            $res = $conn->query(
                                ...
                                "SELECT products.name, products.vendor, SUM(new_sale.quantity) AS quantity, FORMAT(SUM(new_sale.quantity * products.price),2) AS Cost_Price ,FORMAT(SUM(new_sale.quantity * new_sale.sale_price),2) AS Selling_Price,
                                FORMAT((SUM(new_sale.quantity * new_sale.sale_price) - SUM(new_sale.quantity * products.price)),2) AS Profit
                                FROM new_sale
                                INNER JOIN products ON new_sale.product=products.id
                                WHERE `salesman`='$id' AND products.vendor='$store_code' AND new_sale.date BETWEEN CAST('$from' AS DATE) AND DATE_ADD(CAST('$to' AS DATE), INTERVAL 1 DAY)
                                GROUP BY  products.name 
                                ORDER BY products.name"
                            ...
                            //     "SELECT FORMAT(
                            //      SUM(
                            //           SUM(new_sale.quantity * new_sale.sale_price) - SUM(new_sale.quantity * products.price)
                            //       ),2) AS total_profit from new_sale
                            //     INNER JOIN products on products.id=new_sale.product 
                            ...
                            $res = $conn->query(
                                ...
                                FROM new_sale
                                INNER JOIN products ON new_sale.product=products.id
                                WHERE `salesman` IN(SELECT id FROM users) AND products.vendor='$store_code' AND new_sale.date BETWEEN CAST('$from' AS DATE) AND DATE_ADD(CAST('$to' AS DATE), INTERVAL 1 DAY)
                                GROUP BY  products.name 
                                ORDER BY products.name"
                  ...
                  "<tfoot>";
                    ...
                    echo  "<th></th>";
                    echo  "<th></th>";
                    echo  $total_cost_price_ = !isset($tot) ? "<th> 0 </th>" : "<th> ".number_format($tot['TOTAL_COST_PRICE'], 2)."</th>";;
                    echo  $total_selling_price_ = !isset($tot) ? "<th> 0 </th>" : "<th>".number_format($tot['TOTAL_SELLING_PRICE'], 2)."</th>";
                      echo  $total_profit_ = !isset($tot) ? "<th> 0 </th>" : "<th>".number_format($tot['TOTAL_PROFIT'], 2)."</th>";
