                ...
                <input type="date" name="date_from" required value="<?php echo $from = $_POST['date_from'] ? $_POST['date_from'] : date(); ?>" class="form-control">
                  ...
                  <input type="date" name="date_to" required value="<?php echo $to = $_POST['date_to'] ? $_POST['date_to'] : date(); ?>" class="form-control">
                     ...
                     <td> <label>Select Salesman:</label>
		              <select class="form-group" id="id" name="id" required>
                    ...
                    if ($_SERVER["REQUEST_METHOD"] == "POST") {
                        ...
                        if($_POST['id'] == 'all' and $_POST['store_code'] == "all") {
                            ...
                            $qry = $conn->query(
                                ...
                                WHERE `salesman` IN (Select id FROM users) AND date BETWEEN CAST('$from' AS DATE) AND DATE_ADD(CAST('$to' AS DATE), INTERVAL 1 DAY)"
                            ...
                            $res = $conn->query(
                                ...
                                "SELECT * FROM new_sale INNER JOIN products ON new_sale.product=products.id 
                            ...
                            while ($row = $res->fetch_assoc()) {
                                ...
                                echo "<tr>";
                                    ...
                                    $custs = $conn->query("SELECT cust_name FROM customers WHERE id='$customer'") or die($conn->error);
                                    $column = $custs->fetch_array();
                                    echo "<td>".(isset($column['cust_name']) ? $column['cust_name'] : '')."</td>";
                                    echo "<td>".$row['name']."</td>";
                                    echo "<td>".$row['quantity']."</td>";
                                    echo "<td>".$row['sale_price']."</td>";
                                    echo "<td>".$row['quantity']*$row['sale_price']."</td>";
                                    echo '<td>Paid</td>';
                                    $salesguy = $row['salesman'];
                                    $users = $conn->query("SELECT username FROM users WHERE id='$salesguy'") or die($conn->error);
                                    $col = $users->fetch_array();
                            ...
                            $res = $conn->query(
                                ...
                                "SELECT *, products.vendor FROM new_sale INNER JOIN products ON new_sale.product=products.id 
